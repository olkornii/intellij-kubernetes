...
"""
    ...
    private val AZRAEL = PodBuilder(POD3)
        ...
        .withApiVersion("v1")
        .build()
    private val lastPushedPulled = GARGAMEL
    ...
    fun `#isModified should return true if editor resource is changed when compared to pushed resource`() {
        // given
        editor.editorResource = GARGAMEL_WITH_LABEL
        editor.lastPushedPulled = GARGAMEL
        // when
        val modified = editor.isModified()
        // then
        assertThat(modified).isTrue()
    ...
    private class TestableResourceEditor(
        ...
        public override var lastPushedPulled: HasMetadata?
            get() = super.lastPushedPulled
            set(value) { super.lastPushedPulled = value }
    ...
    private fun givenEditorResourceIsModified(modified: Boolean) {
        val lastPulledPushed = GARGAMEL
        val editorResource = if (modified) {
            GARGAMEL_WITH_LABEL
        } else {
            GARGAMEL
        }
        editor.editorResource = editorResource
        editor.lastPushedPulled = lastPulledPushed
